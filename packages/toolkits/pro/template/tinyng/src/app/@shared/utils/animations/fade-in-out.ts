...
export const fadeInOut: AnimationTriggerMetadata = trigger('fadeInOut', [
  ...
  transition('void => bottom', [style({ opacity: 0.8, transform: 'scaleY(0.8) translateY(-4px)' }), animate(`${duration} ${easeOut}`)]),
  transition('bottom => void', [
    animate(`${duration} ${easeIn}`, style({ transform: 'scaleY(0.8) translateY(-4px)', opacity: 0, transformOrigin: '0 0%', display: 'none' })),
  ]),
  transition('void => top', [
  ...
  transition('top => void', [
    animate(`${duration} ${easeIn}`, style({ transform: 'scaleY(0.8) translateY(4px)', opacity: 0, transformOrigin: '0 100%', display: 'none' })),
  ]),
]);
