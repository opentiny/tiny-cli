...
import { AnimationCurves, AnimationDuration } from '../../utils/animations/animationParameters';
import { TProBaseWindowRef } from '../window-ref';
import { TProBaseDropDownDirective } from './dropdown.directive';

@Directive({
  selector: '[tProBaseDropDownMenu]',
  exportAs: 't-pro-base-dropdown-menu',
})
export class TProBaseDropDownMenuDirective implements OnInit, OnDestroy {
  player: AnimationPlayer | undefined;
  @HostBinding('style.display') display = 'none';
  @HostBinding('attr.tabIndex') tabIndex = -1;
  @HostBinding('class.t-pro-base-dropdown-menu') addClass = true;
  ...
  ngOnInit() {
    ...
    this.subscription = this.dropdown.visibleSubject.subscribe((value: any) => {
      if (value !== this.currentValue) {
        ...
        if (value) {
          this.keydownEscapeSub = this.keydownEscapeEvent$.subscribe((event: any) => {
  ...
  calcPopDirection(value: any) {
    const dropdownMenuElement = this.el.nativeElement;
    const elementHeight = dropdownMenuElement.offsetHeight;
    const bottomDistance = this.windowRef.innerHeight ? this.windowRef.innerHeight - this.dropdown.el.nativeElement.getBoundingClientRect().bottom : 0;
