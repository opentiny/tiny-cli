...
import { TProBaseDropDownService } from './dropdown.service';
...
export class TProBaseDropDownDirective implements OnDestroy, OnChanges, AfterContentInit {
  ...
  public set dropDownMenu(dropdownMenu: any) {
    // init drop down menu
    this.menuEl = dropdownMenu.el;
  }

  public set dropDownToggle(dropdownToggle: any) {
    // init toggle element
  ...
  constructor(
    private dropdownService: TProBaseDropDownService,
    private cdr: ChangeDetectorRef,
    public el: ElementRef,
    private devConfigService: DevConfigService,
    @Optional() @SkipSelf() public parentDropdown: TProBaseDropDownDirective,
  ...
  simulateEventDispatch($event: any, target?: any) {
    const event = this.document.createEvent('MouseEvents');
    event.initEvent($event.type, true, true);
    (event as any)['originEvent'] = $event['originEvent'] || $event;
