        ...
        <tiny-row v-if="setCollapse" :flex="true" justify="center" class="col">
          <tiny-col :span="4">
            <tiny-form-item :label="$t('searchTable.columns.workname')">
              <tiny-input
                v-model="filterOptions.workname"
                :placeholder="$t('searchTable.form.input')"
              ></tiny-input>
            </tiny-form-item>
          </tiny-col>
          <tiny-col :span="4" label-width="100px">
            ...
            >
              <tiny-input
                v-model="filterOptions.enablement"
                :placeholder="$t('searchTable.form.input')"
              ></tiny-input>
            </tiny-form-item>
          </tiny-col>
        ...
        <div class="tiny-fullscreen-scroll">
          <div class="tiny-fullscreen-wrapper">
            ...
            >
              ...
              >
                <template #default>
                  <span>{{ $t('searchTable.columns.department') }}</span>
                </template>
              </tiny-grid-column>
              <tiny-grid-column
                field="status"
...
<script lang="ts" setup>
  ...
  import { queryTaskList, QueryTaskParmas } from '@/api/list';
  import {
    Grid as TinyGrid,
  ...
  async function fetchData(
    ...
    state.loading = true;
    try {
      const { data } = await queryTaskList(queryParmas);
