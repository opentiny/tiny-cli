...
export default async () => {
  ...
  const answers: Answer = await inquirer.prompt([
    ...
    },
    {
      ...
      validate: function (value:string, answers:Answer) {
        if (answers.type === 'plugin' && value.indexOf(`${prefix}-plugin-`) !== 0) {
          return `${prefix} 插件名称需要 ${prefix}-plugin- 开头`
        }
        if (answers.type === 'toolkit' && value.indexOf(`${prefix}-toolkit-`) !== 0) {
  ...
  const pluginShortName = answers.type === 'toolkit' ? pluginName.replace( modules.utils.toolkitPrefix() , '') : pluginName.replace( modules.utils.pluginPrefix() , '' )
  ...
  if (!fs.existsSync(path.join(cwd, '.git'))) {
    ...
    spawn.sync('git', ['init']);
    spawn.sync('git', ['remote', 'add', 'origin', `ssh://git@git.huawei.com:2222/${prefix}/${pluginName}.git`]);
    // 初始化后提交一把
    spawn.sync('git', ['add', '*']);
    spawn.sync('git', ['commit', '-m', `temp: [${prefix} auto] init project #IR2019100100120`]);
  }
  ...
  console.log(chalk.green(`${chalk.yellow(`$ ${prefix} start`)}         # 可一键开启项目开发环境`));
  console.log(chalk.green(`${chalk.yellow(`$ ${prefix} help`)}          # 可查看当前套件的详细帮助`));
  console.log(chalk.yellow(`\n--------------------技术支持: ${data.name}--------------------\n`));
};
