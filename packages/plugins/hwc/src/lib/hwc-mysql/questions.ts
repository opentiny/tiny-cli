...
export function getMysqlInstancesQuestions(mysqls: Array<InstanceInfo>) {
  return [
    {
      type: 'table',
      name: 'instance_id',
...
export function databaseQuestions() {
  return [
    {
      name: 'name',
      type: 'input',
      message:
        '请输入数据库名称（长度1-64, 支持字母、数字、中划线、下划线、$, $ 累计总长度小于等于10, 并且MySQL 8.0不可包含$)。',
      validate: (input) => {
        const name = input.trim();
        if (name === '') {
          return '请输入数据库名称';
        }
        return /^[0-9a-zA-Z$_-]{1,64}$/.test(name) &&
          name.split('$').length <= 11
          ? true
          : '数据库名称格式错误';
      },
      replace: '请输入数据库名称',
...
export function getDeleteRuleQuestions(
  securityGroupRulesInfo: Array<SecurityGroupRulesInfo>
) {
  return [
    {
      name: 'deleteSecurityGroupRule',
      type: 'confirm',
      message: `当前安全组规则超出规定限制${SECURITY_GROUP_RULE_LIMIT}条，是否删除已有安全规则？`,
    },
    {
      type: 'table',
      ...
      colWidths: [10, 20, 40],
      valueKey: 'id',
      displayAnswer: (data: SecurityGroupRulesInfo) => data.id,
      columns: [
        {
          name: '序号',
          align: 'center',
      ...
      rows: securityGroupRulesInfo,
      when: (ans) => ans.deleteSecurityGroupRule,
    },
  ];
}
