...
export const createInstance = async (ans: MysqlCreateOption, cliConfig) => {
  ...
  // 由于password参数影响， hcloud会询问。b表示使用命令行中的参数！
  return hcloudClient.exec(cmd, 'b');
};
...
/** 创建数据库 */
export const databaseCreate = async (ans: MysqlCreateOption, cliConfig) => {
  return hcloudClient.exec(`hcloud rds CreateDatabase
  --cli-region="${cliConfig.region.id}"
  --instance_id="${ans['instance_id']}"
  --name="${ans.name}"
  --character_set="${ans['character_set']}"`);
};

/** 查询数据库列表 */
export const queryDatabaseLists = async (instanceId: string, cliConfig) => {
  const res = await hcloudClient.execJson(`hcloud RDS ListDatabases
  --cli-region="${cliConfig.region.id}"
  --instance_id="${instanceId}"
  --limit=100 --page=1`);
  return res.databases as Array<DatabaseInfo>;
};
