...
async function promptCreateDBConfig() {
  const quest = await getCreateDBConfigQuestions();
  const ans = await inquirer.prompt(quest, {});
  if (ans.isCreateDBCfg) {
    await createDBCfg();
  }
}
