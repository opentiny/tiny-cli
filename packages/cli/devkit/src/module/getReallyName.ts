...
export interface ModuleInfo {
	...
	isUseModule: boolean; // 是否使用aio原生模块
...
export default async function (name: string): Promise<ModuleInfo> {
	...
	const aioName = name.replace(scope, DEFAULT_SCOPE).replace(prefix, DEFAULT_BIN);
	...
	if (!exist) {
		...
		if (isCustomPrefix) {
			...
			if (!exist) {
				...
				if (!exist) {
					exist = await onlineExist(aioName);
