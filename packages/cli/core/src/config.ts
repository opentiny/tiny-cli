...
import { cliConfig } from '@opentiny/cli-devkit-core';
...
// 运行前的一些初始化配置工作，这些内容将存于aio的运行时
initConfig(cliEnv);

// Check if we need to profile this CLI run.
if (process.env['AIO_CLI_PROFILING']) {
	let profiler: {
	...
	const exitHandler = (options: { cleanup?: boolean, exit?: boolean }) => {
		if (options.cleanup) {
			...
			fs.writeFileSync(
				path.resolve(process.cwd(), process.env['AIO_CLI_PROFILING'] || '') + '.cpuprofile',
