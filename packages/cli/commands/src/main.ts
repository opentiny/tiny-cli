/**
 * @desc: 所有的套件/插件公共流程
 * 自定义prefix 本地套件/插件 -> aio本地套件/插件 -> 自定义线上套件/插件 -> aio线上套件/插件
 */
import { config, home, Intl, logs, modules, task, cliConfig, error, report } from '@opentiny/cli-devkit-core';
...
const ErrorType = report.ErrorType;
...
class Main {
	...
	public async start(command: string, cliArgs: any) {
		...
		if (typeof toolkitCommand === 'function') {
			...
			log.error(msg);
			report.error(ErrorType.CLI_CORE, msg);
			return;
		}
		...
		if (['start', 'build'].indexOf(command) !== -1) {
			if (toolkitName) {
				...
				// 存在aio.config.js文件且文件中有对应的 start、build、publish时则不需要提示
	...
	/**
	 * 运行插件命令
	 * 运行逻辑：自定义本地插件 -> aio本地插件 -> 自定义线上插件 -> aio线上插件
	 * 先走本地已安装，速度快一些
	 * @param name
	 * @param cliArgs
	 */
	public async runPlugin(name: string, cliArgs: any[]) {
		...
		if (module.exist) {
			...
			log.error(msg);
			report.error(ErrorType.MODULE_NOT_FOUND, msg);
		}
	}

	/**
	 * 展示本地版本号，显示查找逻辑：自定义prefix本地模块 -> aio本地模块
	 */
